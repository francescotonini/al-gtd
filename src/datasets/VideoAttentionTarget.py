...
from src.utils.gaze_ops import get_gaze_fields, get_head_mask, get_label_map
...
class VideoAttentionTarget(Dataset):
    ...
    def _load_gaze_dataset(self):
        ...
        for show_dir in glob.glob(os.path.join(labels_dir, "*")):
            for sequence_path in glob.glob(os.path.join(show_dir, "*", "*.txt")):
                ...
                # Calculate eye_x and eye_y as center of bbox
                df["eye_x"] = (df["xmin"] + df["xmax"]) / 2
                df["eye_y"] = (df["ymin"] + df["ymax"]) / 2
    ...
    def __getitem__(self, index):
        ...
        image = Image.open(os.path.join(self.data_dir, "images", img_path)).convert("RGB")
        img_width, img_height = image.size

        # Load depth image
        depth = Image.open(os.path.join(self.data_dir, "depths", img_path)).convert("L")
        ...
        for _, row in self.full_gaze_annotations.get_group(sample_key).iterrows():
            ...
            gaze_inside.append(gaze_x != -1 and gaze_y != -1)
        ...
        eyes_coords = torch.FloatTensor(eyes_coords)
        gaze_coords = torch.FloatTensor(
            gaze_coords
        )
